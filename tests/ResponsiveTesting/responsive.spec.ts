...
test.describe("Homepage Scenarios - Visual Testing", () => {
  ...
  test("Homepage - header", async ({ pageManager, page }) => {
    await expect(
      page.locator("section.elementor-top-section.header-main")
    ).toHaveScreenshot(`homepage-header-${ENV.PROJECT_NAME as string}.png`);
  });
});
